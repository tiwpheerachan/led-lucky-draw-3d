...
export default function PresenterPage() {
  ...
  // ✅ ผู้ชนะทั้งหมดของรางวัลปัจจุบัน
  const currentPrizeId = safeStr(state?.prize?.prize_id ?? state?.prize?.id ?? "");
  const prizeWinnersMap = state?.prizeWinners ?? {};
  const currentPrizeWinners: Winner[] = currentPrizeId ? (prizeWinnersMap[currentPrizeId] ?? []) : [];
  const prizeQtyNum = Math.max(1, parseInt(safeStr(state?.prize?.qty) || "1") || 1);
  const winnerPosition = currentPrizeWinners.length; // ลำดับคนปัจจุบัน
          ...
          <div className="winnerBanner">
            ...
            <div className="winnerInner">
              ...
              <div className="ribbon">
                🏆 ผู้โชคดี{prizeQtyNum > 1 ? ` คนที่ ${winnerPosition}/${prizeQtyNum}` : "ประจำรอบ"}
              </div>
              ...
              {(winner.team || winner.department) && (
                <div style={{ fontSize: 14, color: "rgba(30,30,60,.55)", marginBottom: 8, fontWeight: 600 }}>
                  {[winner.team, winner.department].filter(Boolean).join(" · ")}
                </div>
              )}
              ...
              <div className="winnerSide">
                ...
                {prizeQtyNum > 1 && (
                  <div className="pill" style={{ background: winnerPosition >= prizeQtyNum ? "linear-gradient(90deg,rgba(16,185,129,.22),rgba(5,150,105,.22))" : "rgba(245,158,11,.12)", borderColor: winnerPosition >= prizeQtyNum ? "rgba(16,185,129,.35)" : "rgba(245,158,11,.30)", color: winnerPosition >= prizeQtyNum ? "rgb(6,95,70)" : "rgb(120,60,0)" }}>
                    {winnerPosition >= prizeQtyNum ? `✅ ครบ ${prizeQtyNum} คน` : `🎯 ${winnerPosition}/${prizeQtyNum} คน`}
                  </div>
                )}
              </div>

              {/* ✅ รายชื่อผู้ชนะก่อนหน้า (เมื่อมี qty > 1) */}
              {prizeQtyNum > 1 && currentPrizeWinners.length > 1 && (
                <div style={{
                  marginTop: 14, padding: "10px 16px", borderRadius: 14,
                  background: "rgba(26,26,46,.06)", border: "1px solid rgba(26,26,46,.10)",
                  width: "100%", maxWidth: 380,
                }}>
                  <div style={{ fontSize: 11, fontWeight: 800, color: "rgba(26,26,46,.45)", marginBottom: 6, textTransform: "uppercase", letterSpacing: 1 }}>
                  ...
                  {currentPrizeWinners.map((w, i) => (
                    <div key={i} style={{
                      ...
                      padding: "3px 0",
                      fontSize: i === currentPrizeWinners.length - 1 ? 15 : 13,
                      fontWeight: i === currentPrizeWinners.length - 1 ? 800 : 600,
                      color: i === currentPrizeWinners.length - 1 ? "rgba(26,26,46,.92)" : "rgba(26,26,46,.55)",
                    }}>
                      <span style={{
                        fontSize: 10, fontWeight: 900, color: "#fff",
                        background: i === currentPrizeWinners.length - 1 ? "#F472B6" : "rgba(26,26,46,.25)",
                        borderRadius: "50%", width: 18, height: 18,
                        display: "inline-flex", alignItems: "center", justifyContent: "center", flexShrink: 0,
                      }}>{i + 1}</span>
                      {w.name || w.participant_id}
                    </div>
                  ))}
                </div>
              )}
