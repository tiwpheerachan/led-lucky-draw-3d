...
export default function AdminPage() {
  ...
  // ✅ prizeWinners อ่านจาก server state โดยตรง (ไม่เก็บ localStorage อีกต่อไป)
  ...
  useEffect(() => {
    ...
    const off = realtime.on((msg) => {
      ...
      if (msg.type === "STOPPING") {
        ...
        // prizeWinners อัปเดตอัตโนมัติผ่าน STATE payload จาก server
      }
    });
    return off;
  }, []);
  ...
  function pressResetPrize(pid: string) {
    if (!pid) return;
    realtime.send("RESET_PRIZE", { prizeId: pid });
    playSound("reset");
  }

  function pressRemoveWinner(pid: string, idx: number) {
    if (!pid || idx < 0) return;
    realtime.send("REMOVE_WINNER", { prizeId: pid, idx });
    playSound("reset");
  }
  ...
  // ✅ prizeWinners จาก server state
  const prizeWinners = state?.prizeWinners ?? {};

  // ✅ จำนวนรางวัล (ชนิด) ที่สุ่มไปแล้วอย่างน้อย 1 คน
  const drawnCount = Object.values(prizeWinners).filter((ws) => ws.length > 0).length;

  // ✅ ผู้ชนะของรางวัลที่กำลังเลือกอยู่
  const prizeQtyNum = Math.max(1, parseInt(prizeQty) || 1);
  const currentPrizeWinners = prizeId ? (prizeWinners[prizeId] ?? []) : [];
  const currentPrizeFull = currentPrizeWinners.length >= prizeQtyNum;
      ...
      <div style={S.shell}>
        ...
        <div style={S.header}>
          <div style={S.headerLeft}>
            ...
            <div style={S.metaRow}>
              ...
              {/* ✅ progress badge — รวมผู้ชนะทั้งหมด */}
              {prizes.length > 0 && (
                <>
                  <div style={S.metaDot}>•</div>
                  <div style={{
                    ...
                    ✅ สุ่มแล้ว <b>{Object.values(prizeWinners).reduce((acc, ws) => acc + ws.length, 0)}</b> คน / <b>{drawnCount}</b> รางวัล
                  </div>
                </>
              )}
            </div>
        ...
        <div className="admGrid" style={S.mainGrid}>
          <section style={S.card}>
            ...
            {/* ✅ Quick number buttons — แสดง progress X/qty */}
            <div style={S.numberGrid}>
              {(prizes as any[]).map((p, idx) => {
                ...
                const pWinners = prizeWinners[id] ?? [];
                const pQtyNum = Math.max(1, parseInt(safeStr(p.qty) || "1") || 1);
                const drawn = pWinners.length > 0;
                const full = pWinners.length >= pQtyNum;
                const badgeLabel = pQtyNum > 1 ? `${pWinners.length}/${pQtyNum}` : "✓";
                const winnerNames = pWinners.map((w: any) => w.name || w.participant_id).join(", ");
                return (
                  <button key={id} type="button"
                    onClick={() => selectPrizeByIndex(idx + 1)}
                    disabled={isSpinning}
                    title={`${safeStr(p.prize_name || p.name || "")}${drawn ? ` ✅ ${winnerNames}` : ""}`}
                    style={{
                      ...S.numBtn,
                      ...(active ? S.numBtnOn : null),
                      ...(full ? S.numBtnDrawn : drawn ? { ...S.numBtnDrawn, background: "rgba(251,191,36,.15)", border: "1.5px solid rgba(251,191,36,.50)" } : null),
                    ...
                    {drawn && (
                      <span style={{
                        ...
                        padding: "0 4px",
                        borderRadius: 999,
                        background: full ? "rgba(16,185,129,1)" : "rgba(251,191,36,1)",
            ...
            <div style={{ marginTop: 8, display: "flex", gap: 14, flexWrap: "wrap", alignItems: "center" }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 5, fontSize: 11, fontWeight: 700, color: "rgba(71,85,105,.85)" }}>
                <span style={{ width: 14, height: 14, borderRadius: 4, background: "rgba(251,191,36,.15)", border: "1px solid rgba(251,191,36,.50)", display: "inline-block" }} />
                สุ่มบางส่วน
              </div>
              <div style={{ display: "flex", alignItems: "center", gap: 5, fontSize: 11, fontWeight: 700, color: "rgba(71,85,105,.85)" }}>
              ...
              <div style={S.selectedInfo}>
                ...
                <div style={S.selectedSub}>
                  ...
                  <span style={{
                    ...
                    background: currentPrizeFull ? "rgba(16,185,129,.12)" : currentPrizeWinners.length > 0 ? "rgba(251,191,36,.15)" : "rgba(15,23,42,.05)",
                    color: currentPrizeFull ? "rgba(10,120,100,1)" : currentPrizeWinners.length > 0 ? "rgba(120,80,0,1)" : "rgba(71,85,105,.70)",
                    border: `1px solid ${currentPrizeFull ? "rgba(16,185,129,.30)" : currentPrizeWinners.length > 0 ? "rgba(251,191,36,.40)" : "rgba(15,23,42,.10)"}`,
                  }}>
                    {currentPrizeFull ? "✅ ครบ qty แล้ว" : `🎯 ${currentPrizeWinners.length}/${prizeQtyNum} คน`}
                  </span>
                </div>

                {/* ✅ รายชื่อผู้ได้รับรางวัลนี้ */}
                {currentPrizeWinners.length > 0 && (
                  <div style={{
                    ...
                    <div style={{ fontSize: 11, fontWeight: 800, color: "rgba(10,100,80,.80)", marginBottom: 2 }}>
                      🏆 ผู้ได้รับรางวัล ({currentPrizeWinners.length}/{prizeQtyNum})
                    </div>
                    {currentPrizeWinners.map((w: any, i: number) => (
                      <div key={i} style={{ fontSize: 12, fontWeight: 700, color: "rgba(15,23,42,.85)", display: "flex", alignItems: "center", gap: 6 }}>
                        <span style={{ fontSize: 10, color: "rgba(16,185,129,1)", fontWeight: 900 }}>#{i + 1}</span>
                        <span style={{ flex: 1 }}>{w.name || w.participant_id}</span>
                        {(w.team || w.department) && (
                          <span style={{ fontSize: 10, color: "rgba(100,116,139,.75)" }}>
                            {[w.team, w.department].filter(Boolean).join(" · ")}
                          </span>
                        )}
                        {/* ✅ ลบเฉพาะคนนี้ออก (กรณีไม่มารับรางวัล) */}
                        <button
                          title={`ลบ ${w.name || w.participant_id} ออกจากรายชื่อผู้ชนะ`}
                          onClick={() => {
                            if (window.confirm(`ลบ "${w.name || w.participant_id}" ออกจากรายชื่อผู้ชนะรางวัล "${prizeName}" ใช่ไหม?\n(สามารถสุ่มใหม่แทนคนนี้ได้)`)) {
                              pressRemoveWinner(prizeId, i);
                            }
                          }}
                          disabled={isSpinning}
                          style={{
                ...
                <div style={S.selectedMiniActions}>
                  ...
                  {currentPrizeWinners.length > 0 && (
                    <button
                      ...
                      onClick={() => {
                        if (window.confirm(`รีเซ็ตผู้ชนะทั้งหมดของรางวัล "${prizeName}" ใช่ไหม?`)) {
                          pressResetPrize(prizeId);
                        }
                      }}
                      disabled={isSpinning || !prizeId}>
                      🔄 รีเซ็ตรางวัลนี้
                    </button>
                  )}
                </div>
            ...
            <div style={S.tableWrap}>
              ...
              <div style={S.tableBodyFlex}>
                {filteredCatalog.map((p: any, idx) => {
                  ...
                  const pWinners = prizeWinners[id] ?? [];
                  const pQtyNum = Math.max(1, parseInt(qty) || 1);
                  const drawn = pWinners.length > 0;
                  const full = pWinners.length >= pQtyNum;
                  ...
                  return (
                    <div key={id} style={{
                      ...
                      <div style={{ ...S.tdStrong, ...(full ? { textDecoration: "line-through", opacity: 0.55 } : null) }}>{name || "—"}</div>
                      ...
                      {/* ✅ สถานะสุ่ม — แสดง X/qty */}
                      <div>
                        {full ? (
                          <span style={{ ...S.tag, ...S.tagOk, fontSize: 11 }}>✅ ครบ {qty} คน</span>
                        ) : drawn ? (
                          <span style={{ ...S.tag, fontSize: 11, background: "rgba(251,191,36,.15)", color: "rgba(120,80,0,1)", border: "1px solid rgba(251,191,36,.40)" }}>
                            🎯 {pWinners.length}/{pQtyNum} คน
                          </span>
                        ) : (
                      ...
                      <div style={{ display: "flex", justifyContent: "flex-end", gap: 4 }}>
                        {drawn && (
                          <button
                            style={{ ...S.btnMini, background: "rgba(239,68,68,.08)", color: "rgba(180,30,30,1)", border: "1px solid rgba(239,68,68,.20)" }}
                            onClick={() => {
                              if (window.confirm(`รีเซ็ตผู้ชนะของรางวัล "${name}" ใช่ไหม?`)) {
                                pressResetPrize(id);
                              }
                            }}
                            disabled={isSpinning}>
            ...
            <div style={S.actionDockWrap}>
              ...
              <div style={{ ...S.actionDockGrid, gridTemplateColumns: "repeat(3, 1fr)" }}>
                ...
                <DockBtn
                  label="START"
                  labelCN="开始"
                  sub={currentPrizeFull ? `ครบ ${prizeQtyNum} คนแล้ว` : "เริ่มหมุน"}
                  active={isSpinning}
                  disabled={!connected || isSpinning || !selectedPrize || currentPrizeFull}
                  onClick={pressStart}
                />
                <DockBtn
                  label="STOP"
