...
const state = {
  ...
  // ✅ ติดตามผู้ชนะแยกตามรางวัล (prizeId → Winner[]) รองรับ qty > 1
  prizeWinners: {},
...
async function computeWinner({ mapping, prizeId }) {
...
wss.on("connection", (ws) => {
  ...
  ws.on("message", async (buf) => {
    ...
    if (type === "STOP_SPIN") {
      ...
      const prizeId = String(state.prize?.prize_id ?? state.prize?.id ?? "").trim();
      const winner = await computeWinner({ mapping, prizeId });

      state.lastWinner = winner;

      // ✅ บันทึกผู้ชนะในรายรางวัล (รองรับ qty > 1)
      if (winner && prizeId) {
        if (!state.prizeWinners[prizeId]) state.prizeWinners[prizeId] = [];
        state.prizeWinners[prizeId].push(winner);
      }
    ...
    // ✅ ลบผู้ชนะรายคนออกจากรางวัล (ไม่ล้างทั้งหมด)
    if (type === "REMOVE_WINNER") {
      const prizeId = String(payload?.prizeId ?? "").trim();
      const idx = Number(payload?.idx ?? -1);
      if (prizeId && state.prizeWinners[prizeId] && idx >= 0 && idx < state.prizeWinners[prizeId].length) {
        state.prizeWinners[prizeId].splice(idx, 1);
        if (state.prizeWinners[prizeId].length === 0) {
          delete state.prizeWinners[prizeId];
        }
      }
      state.spinning = false;
      state.lastWinner = null;
    ...
    // ✅ รีเซ็ตเฉพาะรางวัลที่เลือก (ไม่ล้างทั้งหมด)
    if (type === "RESET_PRIZE") {
      const prizeId = String(payload?.prizeId ?? "").trim();
      if (prizeId) {
        delete state.prizeWinners[prizeId];
    ...
    if (type === "RESET") {
      state.spinning = false;
      state.lastWinner = null;
      state.prizeWinners = {}; // ล้างผู้ชนะทั้งหมด
